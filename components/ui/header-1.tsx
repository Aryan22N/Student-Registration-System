...
import { UserCircle, LogOut, Home, LayoutDashboard } from 'lucide-react';
...
export function Header() {
	...
	if (loading) {
		return (
			<header className="sticky top-0 z-50 w-full border-b border-border/50 bg-background/95 supports-[backdrop-filter]:bg-background/80 backdrop-blur-xl shadow-sm">
				<nav className="mx-auto flex h-16 w-full max-w-6xl items-center justify-between px-6">
					<div className="hover:bg-accent/50 rounded-lg p-2.5 transition-all duration-200 hover:shadow-sm border border-transparent hover:border-border/30">
		...
		<>
			<header
				className={cn(
					...
					{
						'bg-background/95 supports-[backdrop-filter]:bg-background/80 backdrop-blur-xl shadow-lg border-b border-border/60':
							scrolled,
						'bg-background/90 border-b border-border/40 shadow-md': !scrolled,
					}
				)}
			>
				...
				<nav className="relative mx-auto flex h-16 w-full max-w-6xl items-center justify-between px-6">
					...
					<Link 
						href="/" 
						className="group hover:bg-accent/30 rounded-xl p-2.5 transition-all duration-300 hover:shadow-lg border border-transparent hover:border-primary/20 bg-gradient-to-br from-background to-accent/5"
					>
						<div className="flex items-center gap-2">
							<div className="size-8 rounded-lg bg-gradient-to-br from-primary to-primary/70 flex items-center justify-center shadow-inner">
					...
					<div className="hidden items-center gap-3 md:flex">
						<div className="flex items-center gap-1 rounded-2xl bg-accent/10 p-1.5 border border-border/30 backdrop-blur-sm">
							{links.map((link) => (
								<a
									key={link.label}
									className={cn(
										buttonVariants({ variant: 'ghost', size: 'sm' }),
										'rounded-xl px-4 py-2 transition-all duration-200 hover:bg-background/80 hover:shadow-sm border border-transparent hover:border-border/40'
									)}
									href={link.href}
								>
									{link.label}
						...
						<div className="h-6 w-px bg-gradient-to-b from-transparent via-border/50 to-transparent mx-2" />

						{user ? (
							<div className="flex items-center gap-3">
								<div className="flex items-center gap-2 rounded-xl bg-gradient-to-br from-primary/5 to-accent/10 p-2 border border-primary/10 shadow-sm">
									<div className="flex items-center gap-2">
										<div className="size-8 rounded-full bg-gradient-to-br from-primary/20 to-primary/10 border border-primary/20 flex items-center justify-center">
								...
								<Button
									variant="outline"
									className="rounded-xl border-border/40 hover:border-primary/40 hover:bg-primary/5 transition-all duration-200 hover:shadow-sm gap-2"
									asChild
								>
								...
								<Button
									...
									className="rounded-xl border-border/40 hover:border-destructive/40 hover:bg-destructive/5 hover:text-destructive transition-all duration-200 hover:shadow-sm gap-2"
								>
							...
							<div className="flex items-center gap-3">
								<Button
									variant="outline"
									className="rounded-xl border-border/40 hover:border-primary/40 hover:bg-primary/5 transition-all duration-200 hover:shadow-sm"
									asChild
								>
									<Link href="/login">Sign In</Link>
								</Button>
								<Button
									className="rounded-xl bg-gradient-to-r from-primary to-primary/80 hover:from-primary/90 hover:to-primary/70 shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-105 border border-primary/30"
									asChild
								>
					...
					<Button
						...
						className="md:hidden rounded-xl border-border/40 hover:border-primary/40 hover:bg-primary/5 transition-all duration-200"
						aria-expanded={open}
			...
			<MobileMenu open={open} onClose={() => setOpen(false)}>
				<div className="flex flex-col h-full">
					{/* Mobile Menu Header */}
					<div className="p-6 border-b border-border/30 bg-gradient-to-b from-background to-accent/5">
						<div className="flex items-center gap-3">
							<div className="size-10 rounded-xl bg-gradient-to-br from-primary to-primary/70 flex items-center justify-center shadow-inner">
							...
							<div>
								...
								<p className="text-sm text-muted-foreground">Student Event System</p>
							</div>
						</div>
					</div>
						...
						<div className="space-y-4">
							<div className="space-y-2">
								...
								{links.map((link) => (
									<a
										...
										className={cn(
											...
											'w-full justify-start rounded-xl px-4 py-3 hover:bg-accent/30 hover:shadow-sm border border-transparent hover:border-border/30 transition-all duration-200'
										)}
										href={link.href}
								...
								{user && (
									<Button
										variant="ghost"
										className="w-full justify-start rounded-xl px-4 py-3 hover:bg-primary/5 hover:shadow-sm border border-transparent hover:border-primary/20 transition-all duration-200 gap-3"
										asChild
										onClick={() => setOpen(false)}
									>
										<Link href="/create-event">
					...
					{/* Auth Buttons */}
					<div className="p-6 border-t border-border/30 bg-gradient-to-t from-background to-accent/5 space-y-3">
						{user ? (
							<>
								<div className="rounded-xl bg-gradient-to-br from-primary/5 to-accent/10 p-4 border border-primary/10 mb-4">
									<div className="flex items-center gap-3">
										<div className="size-10 rounded-full bg-gradient-to-br from-primary/20 to-primary/10 border border-primary/20 flex items-center justify-center">
								...
								<Button
									variant="outline"
									className="w-full rounded-xl border-border/40 hover:border-destructive/40 hover:bg-destructive/5 hover:text-destructive transition-all duration-200 py-3 gap-3"
									onClick={handleLogout}
								>
									<LogOut className="size-4" />
									Logout
							...
							<>
								<Button
									variant="outline"
									className="w-full rounded-xl border-border/40 hover:border-primary/40 hover:bg-primary/5 transition-all duration-200 py-3"
									asChild
									onClick={() => setOpen(false)}
								>
									<Link href="/login">Sign In</Link>
								...
								<Button
									className="w-full rounded-xl bg-gradient-to-r from-primary to-primary/80 hover:from-primary/90 hover:to-primary/70 shadow-lg hover:shadow-xl transition-all duration-300 py-3"
									asChild
									onClick={() => setOpen(false)}
								>
									<Link href="/login">Get Started</Link>
...
function MobileMenu({ open, children, className, onClose, ...props }: MobileMenuProps) {
	...
	return createPortal(
		<div className="fixed inset-0 z-40">
			...
			{/* Menu Container */}
			<div className="relative h-full max-w-md ml-auto">
				<div
					className={cn(
						'absolute inset-y-0 right-0 w-full bg-background border-l border-border/50 shadow-2xl',
